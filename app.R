...
recipes <- yaml::read_yaml(here::here("recipes.yml"))

recipe_raw <- rio::import(here::here("recipes", "hario_switch_chilled_extraction_recipe.csv")) |>
  mutate(
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      sliderInput("ratio", "Ratio (1:X)", min = 10, max = 20, value = recipes[[1]]$ratio, step = 0.5),
...
server <- function(input, output, session) {
  ...
  show_recipe <- reactive({
    ...
    recipe <- recipe_raw |>
      ...
      add_row(Time = recipes[[1]]$time, Step = "Brewing finished", Note = "")
  })
